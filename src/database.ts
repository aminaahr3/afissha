...
export async function initDatabase(): Promise<void> {
  ...
  try {
    ...
    if (fs.existsSync(initSqlPath)) {
      ...
      console.log("⚠️ init.sql not found at", initSqlPath, "- running inline schema + seed");
      await runInlineSchema(p);
    }
    ...
    console.log(`📊 Data: ${catCount.rows[0].count} categories, ${cityCount.rows[0].count} cities, ${tmplCount.rows[0].count} event templates`);

    if (parseInt(catCount.rows[0].count) === 0) {
      console.log("⚠️ No seed data found, inserting...");
      await runSeedData(p);
      const catCount2 = await p.query("SELECT COUNT(*) FROM categories");
      const cityCount2 = await p.query("SELECT COUNT(*) FROM cities");
      const tmplCount2 = await p.query("SELECT COUNT(*) FROM event_templates");
      console.log(`✅ Seed complete: ${catCount2.rows[0].count} categories, ${cityCount2.rows[0].count} cities, ${tmplCount2.rows[0].count} event templates`);
    }
  } catch (error: any) {
...
async function runInlineSchema(p: Pool): Promise<void> {
  await p.query(`
    ...
    SELECT '', '', '', true WHERE NOT EXISTS (SELECT 1 FROM payment_settings);
    INSERT INTO site_settings (support_contact, support_label, chat_script)
    SELECT 'https://t.me/support', 'Тех. поддержка',
    '<!-- LiveChat --><script>window.__lc=window.__lc||{};window.__lc.license=19416545;</script>'
...
async function runSeedData(p: Pool): Promise<void> {
  ...
  await p.query(`
    INSERT INTO event_templates (id, name, description, category_id, ticket_image_url, is_active, image_url) VALUES
      (1, 'Сальвадор Дали & Пабло Пикассо', 'Откройте мир двух гениев искусства и вдохновитесь их шедеврами. Уникальная экспозиция для всех любителей живописи.', 6, 'https://artchive.ru/res/media/img/oy800/exposition/c22/678496@2x.jpg', true, NULL),
      (2, 'VR Gallery', 'Полное погружение в виртуальные галереи с интерактивными экспонатами. Перенеситесь внутрь картин и создайте свои впечатления.', 6, 'https://www.marinabaysands.com/content/dam/revamp/ASMrevamp/VRgallery/VR-Gallery-Photo-Shoot-1-800x490.jpg', true, NULL),
      (3, 'Реальный космос', 'Путешествие по Вселенной в интерактивной выставке. Узнайте о планетах, звёздах и космосе своими глазами.', 6, 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTbou2jpU3dM2uViVkFfBxFw_vmZsD54TMMdQ&s', true, NULL),
      (4, 'Энди Уорхол и русское искусство', 'Поп-арт встречает классику: яркие работы и современные интерпретации. Идеально для любителей искусства и фотографий.', 6, 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTRCG3ocLudpB89cv9srmMsuizRDpASa5HJvw&s', true, NULL),
      (5, 'Айвазовский. Кандинский. Живые полотна', 'Ожившие шедевры на мультимедийных экранах. Впечатляющее шоу для всей семьи.', 6, 'https://s3.afisha.ru/mediastorage/22/5e/a84badf567224497bf0dac225e22.jpg', true, NULL),
      (6, 'Музей восковых фигур', 'Реалистичные фигуры знаменитостей и исторических личностей. Сделайте фото с кумирами.', 7, 'https://kuda-kazan.ru/uploads/5acced441044b6f1ca31fe364a3003dd.jpg', true, NULL),
      (7, 'VR Музей', 'Интерактивные выставки в виртуальной реальности. Исследуйте экспозиции, как будто внутри них.', 7, 'https://vinchi-interactive.ru/wp-content/uploads/2023/08/vr-111-min.jpg', true, NULL),
      (8, 'Музей истории оружия', 'Коллекция оружия от древности до современности. Узнайте историю через уникальные экспонаты.', 7, 'https://upload.wikimedia.org/wikipedia/commons/thumb/1/13/Weapon_history_museum_39.jpg/1200px-Weapon_history_museum_39.jpg', true, NULL),
      (9, 'Музей драгоценностей', 'Редкие украшения и драгоценные камни. Погрузитесь в мир роскоши и красоты.', 7, 'https://it.latuaitalia.ru/wp-content/uploads/2016/04/comune.vicenza.it_2.jpg', true, NULL),
      ...
      (20, 'Прогулка на лошадях', 'Конная прогулка по живописным маршрутам с инструктором.', 9, 'https://xn--80aagidgbdm1a7a.xn--p1ai/wp-content/uploads/2021/05/5e4o9PQ2aRY-1024x683.jpg', true, NULL),
  ...
  await p.query(`
    INSERT INTO event_template_images (id, event_template_id, image_url, sort_order) VALUES
      ...
      (26, 20, 'https://xn--80aagidgbdm1a7a.xn--p1ai/wp-content/uploads/2021/05/5e4o9PQ2aRY-1024x683.jpg', 0),
